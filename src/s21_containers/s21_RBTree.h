#ifndef CPP2_SRC_CONTAINERS_SRC_RBTREE
#define CPP2_SRC_CONTAINERS_SRC_RBTREE
...
class Branch {
 public:
  ...
  Branch<KeyType, ValueType> *head_;
  ...
  Iterator &operator++() {
    if (node_ != nullptr) {
      if (node_->right_ != nullptr) {
        ...
        Branch<KeyType, ValueType> *parent = node_->head_;
...
class RBTree {
 public:
  RBTree() : root(nullptr), inserted(false){};
  RBTree(const RBTree &other) : root(CopyTree(other.GetRoot())){};
  ...
  RBTree<KeyType, ValueType> &operator=(RBTree &&other) noexcept {
    if (this != &other) {
      Clear(root);
      root = other.root;
      other.root = nullptr;
  ...
  void Swap(RBTree<KeyType, ValueType> &other) { std::swap(root, other.root); };
  ...
  Branch<KeyType, ValueType> *Remove_node(Branch<KeyType, ValueType> *node,
                                          KeyType key) {
 ...
 private:
  ...
  void UpdateSize(Branch<KeyType, ValueType> *node) {
    if (node) {
      node->scale_ = 1 + GetSize(node->left_) + GetSize(node->right_);
  ...
  Branch<KeyType, ValueType> *Remove(Branch<KeyType, ValueType> *node,
                                     KeyType key) {
    if (!node) return nullptr;  // Базовый случай

    // Сравниваем ключи и ищем узел для удаления
    if (key < node->key_) {
      ...
      if (!node->left_ && !node->right_) {
        delete node;  // Правильное освобождение памяти
        return nullptr;  // Узел удален
      } else if (!node->left_) {
...
#endif  // CPP2_SRC_CONTAINERS_SRC_RBTREE
