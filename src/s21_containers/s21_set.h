#ifndef CPP2_SRC_CONTAINERS_S21_SET
#define CPP2_SRC_CONTAINERS_S21_SET
...
class Set {
 public:
  ...
  int size() {
    ...
    return (root != nullptr) ? root->scale_ : 0;
  };
  ...
  void erase(iterator pos) {
    if (pos != nullptr) {
      tree_.Remove_node(tree_.GetRoot(), *pos);
    }
  };
  ...
  const RBTree<KeyValue, KeyValue>& get_tree() const { return tree_; };

 protected:
  RBTree<KeyValue, KeyValue> tree_;
  Allocator allocator;
};

}  // namespace s21
#endif  // CPP2_SRC_CONTAINERS_S21_SET